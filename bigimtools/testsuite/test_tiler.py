...
def test_round_trip(tile_size, overlap):

    tiles = dict(tiler.split_into_tiles(IM_CAMERA, tile_size, overlap))
    ...
    sh = IM_CAMERA.shape
    np.testing.assert_almost_equal(merge[: sh[0], : sh[1]], IM_CAMERA)


@pytest.mark.parametrize("tile_size", ((10, 10), (10, 23), (10, 23)))
@pytest.mark.parametrize("overlap", ((5, 5), (3, 6), (6, 3)))
@pytest.mark.parametrize("init", ((1, 1), (5, 10), (10, 5)))
def test_equalize_simple1(tile_size, overlap, init):
    tiles = dict(tiler.split_into_tiles(IM_CAMERA, tile_size, overlap))
...
@pytest.mark.parametrize("overlap", ((5, 5), (3, 6), (6, 3)))
@pytest.mark.parametrize("init", ((1, 1), (5, 10), (10, 5)))
def test_equalize_simple2(tile_size, overlap, init):
    tiles = dict(tiler.split_into_tiles(IM_CAMERA, tile_size, overlap))
    corrections = tiler.equalize_tiles(
...
@pytest.mark.parametrize("overlap", ((5, 5), (3, 6), (6, 3)))
@pytest.mark.parametrize("init", ((1, 1), (5, 10), (10, 5)))
def test_equalize(tile_size, overlap, init):
    tiles = dict(tiler.split_into_tiles(IM_CAMERA, tile_size, overlap))
    merge = tiler.join_tiles(tiles, overlap, tiler.ConstantDict(13))

    sh = IM_CAMERA.shape
    np.testing.assert_almost_equal(
        merge[: sh[0], : sh[1]], 13 * IM_CAMERA
    )
...
@pytest.mark.parametrize("overlap", ((5, 5), (3, 6), (6, 3)))
@pytest.mark.parametrize("init", ((1, 1), (5, 10), (10, 5)))
def test_equalize_change_init(tile_size, overlap, init):
    ...
    sh = IM_CAMERA.shape
    np.testing.assert_almost_equal(
        merge[: sh[0], : sh[1]], 13.0 * IM_CAMERA
    )
...
@pytest.mark.parametrize("overlap", ((5, 5), (3, 6), (6, 3)))
@pytest.mark.parametrize("init", ((1, 1), (5, 10), (10, 5)))
def test_equalize_changed(tile_size, overlap, init):
